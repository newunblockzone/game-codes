...
for (i = 0; i < specificPartnersArr.length; i++) {
    if (loc.indexOf(specificPartnersArr[i]) != -1) showSGLinks = false;
}
...
MLE.sounds = {
    ...
    bounce_01: new Howl({
        src: [
            'sounds/bounce_01.mp3',
        ]
    }),
    bounce_02: new Howl({
        src: [
            'sounds/bounce_02.mp3',
        ]
    }),
    bounce_03: new Howl({
        src: [
            'sounds/bounce_03.mp3',
        ]
    }),
    bounce_04: new Howl({
        src: [
            'sounds/bounce_04.mp3',
        ]
    }),
    bounce_05: new Howl({
        src: [
            'sounds/bounce_05.mp3',
        ]
    }),
    score: new Howl({
        src: [
...
MLE.playSound = function (soundName) {
    if (MLE.soundOn) {
        switch (soundName) {
            case "bounce_01":
                if (MLE.bouncingSound)
                    MLE.sounds.bounce_01.play();
                break;
            case "bounce_02":
                if (MLE.bouncingSound)
                    MLE.sounds.bounce_02.play();
                break;
            case "bounce_03":
                if (MLE.bouncingSound)
                    MLE.sounds.bounce_03.play();
                break;
            case "bounce_04":
                if (MLE.bouncingSound)
                    MLE.sounds.bounce_04.play();
                break;
            case "bounce_05":
                if (MLE.bouncingSound)
                    MLE.sounds.bounce_05.play();
                break;
            case "buttonClickSound":
                MLE.sounds.buttonClickSound.play();
                break;
            case "score":
                MLE.sounds.score.play();
                break;
        }
    }

    if (MLE.musicOn) {
        if (!MLE.sounds.musicSound.playing()) {
            MLE.sounds.musicSound.pause();
            MLE.sounds.musicSound.loop(true);
            MLE.sounds.musicSound.play();
        }
    }
};

MLE.pauseSound = function (soundName) {
    switch (soundName) {
        case "musicSound":
            MLE.sounds.musicSound.pause();
            break;
...
MLE.startLoader = function () {
    ...
    window.addEventListener("blur", () => {
      ...
      //  MLE.sounds.score.volume(0);
      Howler.volume(0);
    });
    window.addEventListener("focus", () => {
      //  if (saveState.data) MLE.soundOn = saveState.data.soundOn;
      //  MLE.sounds.score.volume(1);
      if (saveState.data.soundOn){
        Howler.volume(1);
    }
    });
    MLE.gameLoop();
...
PreLoaderScene.prototype.update = function () {
    ...
    if (this.percent !== 100) {
        ...
        sdkHandler.trigger("loading.update", {
            progressPercentage: this.percent,
        })
...
MenuButton.prototype.onUp = function () {
    if (this.canBePressed) this.clickCallback(), this.canBePressed = false, this.buttonUp.alpha = 1, this.buttonDown.alpha = 0, MLE.playSound('buttonClickSound');
};
...
MainMenu.prototype.init = function () {
    ...
    var a = 0,
        b = a = 0,
        d = 0;
    saveState.data["FurEye.BasketBallHoops.Level1"] &&
        (a = Number(saveState.data["FurEye.BasketBallHoops.Level1"]));
    saveState.data["FurEye.BasketBallHoops.Level2"] && (b = Number(saveState.data["FurEye.BasketBallHoops.Level2"]));
    saveState.data["FurEye.BasketBallHoops.Level3"] && (d = Number(saveState.data["FurEye.BasketBallHoops.Level3"]));
...
GameScene.prototype.sensorTouched = function () {
    ...
    this.netAnimation();
    setTimeout(MLE.bounceFunc, 800);
    setTimeout(MLE.bounceFunc, 1500);
...
GameScene.prototype.pauseClicked = function () {
    MLE.paused === true ? this.resume() : (MLE.paused = true, this.aimArrow.setPaused(true), this.pauseWindow.show(), MLE.bouncingSound = false)
};
...
GameScene2.prototype.pauseClicked = function () {
    MLE.paused === true ? this.resume() : (MLE.paused = true, this.aimArrow.setPaused(true), this.pauseWindow.show())
};
...
Ball.prototype.makeShoot = function (a, b, d) {
    if (MLE.bounceInterval) {
        clearInterval(MLE.bounceInterval);
        MLE.bounceInterval = undefined;
    }
    // MLE.playSound('letterSelectSound');
...
MLE.boundIndex = 1;
MLE.bounceFunc = () => {
    if (MLE.paused) return;
    MLE.playSound('bounce_0' + MLE.boundIndex);
    MLE.boundIndex++;
    if (MLE.boundIndex === 6) MLE.boundIndex = 1;
}
MLE.firstLoad = true;
Ball.prototype.startJumping = function () {
    ...
    MLE.bounceInterval = setInterval(MLE.bounceFunc, 700);
...
GUI.prototype.init = function (a) {
    ...
    saveState.data[a] ? this.bestNumber = Number(saveState.data[a]) : (saveState.data[a] = "0", this.bestNumber = 0);
    saveState.save();
    this.alpha = 1;
...
GUI.prototype.addPoints = function (a) {
    ...
    this.pointsNumber + a > this.bestNumber && (saveState.data[b] = this.pointsNumber + a + "");
    saveState.save();
    this.inRow += 1;
    if (this.inRow === 4) this.inRow = 0;
    // SG_Hooks.levelUp(this.multiplier, this.pointsNumber + a)
};
GUI.prototype.zeroPoints = function () {
    this.inRow += 1;
    // SG_Hooks.levelFinished(this.multiplier, this.pointsNumber);
    if (this.inRow === 4) this.inRow = 0;
    //  SG_Hooks.gameOver(this.multiplier, this.pointsNumber);
    sdkHandler.trigger("gameOver", {
        score: this.pointsNumber,
    })
    createjs.Tween.get(this).to({
        pointsNumber: 0
    }, 750, createjs.Ease.sineInOut)
...
PauseWindow.prototype.init = function () {
    ...
    this.soundButton = new MenuButton("button", saveState.data.soundOn ? currLang.soundOn : currLang.soundOff);
    this.soundButton.group.x = MLE.width / 2;
    this.soundButton.group.y = 630;
    this.soundButton.group.alpha = 0;
    this.soundButton.clickCallback = this.soundClicked.bind(this);
...
PauseWindow.prototype.update = function () {
    this.sprite.alpha === 1 && (this.resumeButton.update());
    this.sprite.alpha === 1 && (this.soundButton.update());
};
...
PauseWindow.prototype.show = function () {
    ...
    createjs.Tween.get(this.soundButton.group).wait(200).to({
        alpha: 1
    }, 350);
...
PauseWindow.prototype.hide = function () {
    ...
    createjs.Tween.get(this.soundButton.group).to({
        alpha: 0
    }, 350);
...
PauseWindow.prototype.soundClicked = function () {
    saveState.data.soundOn = !saveState.data.soundOn;
    //MLE.soundOn = saveState.data.soundOn;
    Howler.volume(saveState.data.soundOn ? 1 : 0);
    saveState.save();
    this.soundButton.text.text = saveState.data.soundOn ? currLang.soundOn : currLang.soundOff;
};
...
var isSmall, remote = {
    ...
    windowOnload = function () {
        ...
        // b = SG_Hooks.getLanguage("en");
        b = sgSettings.config.env.locale;
...
window.initSdk = function () {
    sgSdk.initialize(['basic', 'scoreGame'], {
        ...
        build: '1.0.0',
        supportedLanguages: ['en', 'de', 'es', 'fr', 'it', 'pt', 'ru', 'tr', 'ko', 'pl', 'ja'],
        id: 'basketball-hoops',

        freezeGame: function () { },
        unfreezeGame: function () { },
        runGame: function () {
            MLE.nextGame = new GameScene;
        },
        getScore: function () { },
        startOver: function () { },
    ...
    function save() {
        return new Promise(function (resolve, reject) {
            sdkHandler.trigger('save', {
                key: "basketball-hoops",
                value: JSON.stringify(saveState.data),
                callback: (err) => {
                    if (err) reject(err);
                    else resolve();
                },
            ...
            sdkHandler.trigger("restore", {
                ...
                callback: (error, value) => {
                    if (error) reject(error);
                        ...
                        saveState.data = JSON.parse(value) || {};
                        if (saveState.data.soundOn === undefined) {
                            saveState.data.soundOn = true;
                        }
                        resolve();
                    }
                }
